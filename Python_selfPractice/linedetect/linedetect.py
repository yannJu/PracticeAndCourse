...
class LineDetector:
    ...
    def sliding_window(self,x_start_L,x_start_R,img):
        ...
        if line_flag != 4:
            ...
            for window in range(0, nwindows):
                ...
                if lx_current and  rx_current:
                    cv2.circle(out_img,(int(lx_current),int(ly_current-window*window_height-3)),3,(0,0,255),-1)
                    cv2.circle(out_img,(int(rx_current),int(ry_current-window*window_height-3)),3,(0,0,255),-1)
                    self.length = int(math.sqrt(pow(rx_current - lx_current, 2) + pow (((ry_current-window*window_height-3) - (ly_current-window*window_height-3)), 2)))
                    mean_x = (lx_current + rx_current)/2
                    cv2.circle(out_img,(int(mean_x), int(ry_current-window*window_height-3)),3,(0,255,255),-1)
                    self.xlocate = (int(mean_x), int(ry_current-window*window_height-3))
                    point_list_left.append((lx_current, ly_current-window*window_height-3))
                    point_list_right.append((rx_current,ry_current-window*window_height-3))
                elif lx_current:
                    cv2.circle(out_img,(int(lx_current),int(ly_current-window*window_height-3)),3,(0,0,255),-1)
                    mean_x = (lx_current + width/2)
                    cv2.circle(out_img,(int(mean_x),int(ly_current-window*window_height-3)),3,(0,255,255),-1)
                    self.xlocate = (mean_x,ly_current-window*window_height-3)
                    point_list_left.append((lx_current, ly_current-window*window_height-3))
                elif rx_current:
                    # cv2.circle(out_img,(lx_current,ly_current-window*window_height-3),3,(0,0,255),-1)
                    cv2.circle(out_img,(int(rx_current),int(ry_current-window*window_height-3)),3,(0,0,255),-1)
                    ...
                    cv2.circle(out_img,(int(mean_x),int(ry_current-window*window_height-3)),3,(0,255,255),-1)
                    self.xlocate = (mean_x,ry_current-window*window_height-3)
                    # point_list_left.append((lx_current, ly_current-window*window_height-3))
                    point_list_right.append((rx_current,ry_current-window*window_height-3))
                if line_flag == 3:
                    ...
                    cv2.rectangle(out_img, (int(l_win_x_low), int(l_win_y_low)), (int(l_win_x_high), int(l_win_y_high)), (0, 255, 0), 1)
                    cv2.rectangle(out_img, (int(r_win_x_low), int(r_win_y_low)), (int(r_win_x_high), int(r_win_y_high)), (255,0, 0), 1)
                    ...
                    # draw rectangle
                    # 0.33 is for width of the road
                    cv2.rectangle(out_img, (int(win_x_low), int(win_y_low)), (int(win_x_high), int(win_y_high)), (0, 255, 0), 1)
                    ...
                    cv2.rectangle(out_img, (int(win_x_low), int(win_y_low)), (int(win_x_high), int(win_y_high)), (255, 0, 0), 1)
                    ...
                    if len(good_right_inds) > minpix:
                        ...
                        rx_current = np.int(np.mean(nonzerox[good_right_inds]))
                #cv2.circle(out_img, (int(x_location), int(ry_current-window*window_height-3)), 3, (155, 155, 155), 2)
    ...
    def main(self,img):
        self.x_start_l,self.x_start_r = self.findpoint.findpoint(img)
        output , x_location, point_list_left, point_list_right = self.sliding_window(self.x_start_l,self.x_start_r,img)

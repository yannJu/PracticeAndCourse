...
class SlideWindow:
    ...
    def h_slidewindow(self, img, x_left_start, x_right_start, flag):
        ...
        for i in range(n_windows):
            cv2.rectangle(output_img, (x_left - window_width, y_start - i * window_height),
                          (x_left + window_width, y_start - (i + 1) * window_height), (0, 255, 255), 1)
            cv2.rectangle(output_img, (x_right - window_width, y_start - i * window_height),
                          (x_right + window_width, y_start - (i + 1) * window_height), (255, 0, 255), 1)
            ...
            if find_left and find_right:
                ...
                center_point = x_left + 120
            elif find_right:
                center_point = x_right - 120
        ...
        return output_img, steer_theta, center, x_left, x_right
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            if tf:
                h_img, theta = line.h_slidewindow(dila, int(startL), int(startR), flag)
                oldL = startL
                oldR = startR
            # else:
            #     h_img, theta = line.h_slidewindow(dila, int(oldL), int(oldR), flag)
